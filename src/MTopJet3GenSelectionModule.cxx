...
class MTopJet3GenSelectionModule : public ModuleBASE {

public:
  explicit MTopJet3GenSelectionModule(uhh2::Context&);
...
protected:
  ...
  Event::Handle<std::vector<GenTopJet>>h_gen3fatjets;
  ...
  std::unique_ptr<Hists> h_passSel;
...
MTopJet3GenSelectionModule::MTopJet3GenSelectionModule(uhh2::Context& ctx){

  debug = string2bool(ctx.get("Debug","false")); // look for Debug, expect false if not found

  if(debug) cout << "debug works" << endl;
  ...
  h_gen3fatjets=ctx.get_handle<std::vector<GenTopJet>>("genXCone3TopJets");
  ...
  if(isMC) remove_lepton_gen33.reset(new RemoveLeptonGen(ctx, "genXCone3TopJets"));
  ...
  h_passSel.reset(new GenHists_GenOnly(ctx, "passSel", "genXCone3TopJets"));
...
bool MTopJet3GenSelectionModule::process(uhh2::Event& event){
  ...
  std::vector<GenTopJet> gen3jets = event.get(h_gen3fatjets);

  if(debug) {
    cout << "process start---" << endl;
    for(uint i=0; i< gen3jets.size(); i++) {
      cout << i << " tau1: " << gen3jets.at(i).tau1() << endl;
      cout << i << " tau2: " << gen3jets.at(i).tau2() << endl;
      cout << i << " tau3: " << gen3jets.at(i).tau3() << endl;
      cout << i << " tau4: " << gen3jets.at(i).tau4() << endl;
    }
  } 
  ...
  if(debug) {
    cout << "after cleaner---" << endl;
    gen3jets = event.get(h_gen3fatjets);
    for(uint i=0; i< gen3jets.size(); i++) {
      cout << i << " tau1: " << gen3jets.at(i).tau1() << endl;
      cout << i << " tau2: " << gen3jets.at(i).tau2() << endl;
      cout << i << " tau3: " << gen3jets.at(i).tau3() << endl;
  ...
  if(debug) {
    cout << "pre lepton selection---" << endl;
    gen3jets = event.get(h_gen3fatjets);
    for(uint i=0; i< gen3jets.size(); i++) {
      cout << i << " tau1: " << gen3jets.at(i).tau1() << endl;
      cout << i << " tau2: " << gen3jets.at(i).tau2() << endl;
  ...
  if(!event.isRealData){
    if(gen3jets.size() < 3) return false;
  }

  if(debug) {
    cout << "pre removing of leptons---" << endl;
    gen3jets = event.get(h_gen3fatjets);
    for(uint i=0; i< gen3jets.size(); i++) {
      cout << i << " tau1: " << gen3jets.at(i).tau1() << endl;
      cout << i << " tau2: " << gen3jets.at(i).tau2() << endl;
...
UHH2_REGISTER_ANALYSIS_MODULE(MTopJet3GenSelectionModule)
